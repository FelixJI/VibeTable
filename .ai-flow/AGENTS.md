...
本仓库 Python 入口统一使用 `uv run python ...`；保留根 AGENTS.md 的项目约束与质量门禁。
...
- 精确暂存，提交前执行 `uv run python .ai-flow/scripts/hygiene.py --staged`；推送前核对实际拟推送范围和现有 hooks。守卫不是密钥扫描器或不可绕过的安全边界。
