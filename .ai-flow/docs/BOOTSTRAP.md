...
```text
uv run --project "<仓库根目录>" python install.py --repo "<仓库根目录>" --upgrade
uv run --project "<仓库根目录>" python install.py --repo "<仓库根目录>" --upgrade --apply --confirm-agents-stopped
```

全新安装省略 `--upgrade`。脚本默认 dry-run；`--confirm-agents-stopped` 只确认你已完成上一节检查。

已知旧版文件按哈希识别后备份并替换/删除；未知定制文件不直接覆盖/删除，生成 ignored 升级候选并报告冲突。退出码 2 / unresolved 不等于升级完成，必须继续语义合并。
...
```text
uv run python .ai-flow/scripts/hygiene.py --tracked
uv run python .ai-flow/scripts/hygiene.py --migrate
uv run python .ai-flow/scripts/hygiene.py --migrate --apply
```
...
```text
uv run --project "<仓库根目录>" python -m unittest discover -s tests -v
uv run --project "<仓库根目录>" python install.py --repo "<仓库根目录>" --check
```
