...
提交前 `uv run python .ai-flow/scripts/hygiene.py --staged`。可选安装本地 hooks：`uv run python .ai-flow/scripts/hygiene.py --install-hooks`；不得覆盖现有 hooks 或 hooksPath。已有管理器需显式整合 guard；pre-push 的标准输入应完整保留。
