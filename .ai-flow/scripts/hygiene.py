...
def install_hooks(root):
    ...
    marker = "# AI-FLOW-ARTIFACT-GUARD v4.0"
    planned = []
    for name, flag in [("pre-commit", "--staged"), ("pre-push", "--pre-push")]:
        target = hooks / name
        # Python path can contain spaces; Git for Windows runs hooks using its POSIX sh.
        data = (
            "#!/bin/sh\n"
            + marker
            ...
            + ' "$root/.ai-flow/scripts/hygiene.py" --repo "$root" '
            + flag
            + "\n"
        )
        if target.is_symlink() or (
            target.exists() and target.read_text(encoding="utf-8", errors="replace") != data
        ):
            raise ValueError(
