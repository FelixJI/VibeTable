...
@pytest.mark.parametrize(
    "failure",
    [None, "missing", "mismatch", "failed", "timeout", "close", "exchange", "copy", "report"],
)
def test_replica_conflict_transports_only_closed_successful_stages(
    ...
    def node_run(command, **kwargs):
        ...
        if stage != "fork-left" or failure != "missing":
            result = {
                "scenario": directory_replica_conflict.SCENARIO_ID,
                "status": "failed" if stage == "fork-left" and failure == "failed" else "passed",
    ...
    expected = ["seed", "fork-left", "fork-right", "resolve"]
    if failure not in {None, "report"}:
        expected = (
            expected[:2]
    ...
    assert len(scopes) == (6 if failure in {None, "report"} else 4)
    assert all(scope.closed >= 1 for scope in scopes)
    assert len(set(controls)) == len(controls)
    if failure in {None, "report"}:
        assert all((control / runner.NORMAL_CLOSE_CONTROL_FILE).is_file() for control in controls)
    else:
...
def test_document_diff_scenario_uses_closed_ui_operation_and_rejects_raw_materialize() -> None:
    ...
    assert 'rawMaterializeFailure.includes("workspace.method_not_public")' in scenario
    assert (
        'acknowledgeExpectedBridgeFailureByCodeIfPresent(page, "CAPABILITY_NOT_PUBLIC")'
        not in scenario
    )
