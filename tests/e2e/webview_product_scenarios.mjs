...
async function scenario24(page, recorder, _network, runtime) {
  ...
  if (stage === "seed") {
    ...
    if (!["fork-left", "fork-right", "resolve"].includes(stage)) {
  ...
  recorder.check("the exchanged fork becomes visible through public conflict refresh",
    conflicts.result.conflicts.length > 0, { conflicts });
  await conflictCenter.locator(".conflict-row").first().waitFor({ timeout: 60_000 });
  const inspected = await replicaUiMethod(page, recorder, "conflict.inspect",
    () => conflictCenter.locator(".conflict-row").first().click());
  const tableItem = inspected.result.items?.find((item) => item.kind === "table"
    && item.itemId === state.tableId && item.path === state.tableName);
  recorder.check("real directory forks expose the seeded table in Conflict Center",
    uuid(inspected.result.conflictId) && Boolean(tableItem), { inspected });
  // The minimal seed has one table; any extra conflict needs an explicit scenario decision.
  recorder.check("the conflict contains only the intended table choice",
    inspected.result.items.length === 1, { items: inspected.result.items });
  ...
  recorder.check("public preview freezes exactly the selected remote table choice",
    preview.result.valid === true && uuid(preview.result.planId)
      && preview.request.payload.params.conflictId === inspected.result.conflictId
  ...
  await page.getByTestId("nav-settings").click();
  await page.getByTestId("settings-nav-versions").click();
  for (const snapshotId of applied.result.recoverySnapshotIds) {
    ...
    recorder.check("each returned recovery snapshot is publicly listed and verified",
      snapshot?.integrity === "verified", { snapshotId, snapshot });
