...
def build_renderer_public_method_scopes(policy_path: Path = POLICY) -> dict[str, str]:
    manifest = build_manifest(policy_path)
    methods = manifest["methods"]
    if not isinstance(methods, list):  # pragma: no cover - guarded by build_manifest
        raise ValueError("workspace RPC capability manifest methods must be an array")
    return {
        entry["method"]: entry["scope"]
        for entry in methods
        if isinstance(entry, dict)
        and entry.get("audience") == "rendererPublic"
        and isinstance(entry.get("method"), str)
        and isinstance(entry.get("scope"), str)
    }


def encode_renderer_public_method_scopes(policy_path: Path = POLICY) -> str:
    scopes = json.dumps(
        build_renderer_public_method_scopes(policy_path),
        ensure_ascii=False,
        indent=2,
    )
    return (
        ...
        "// Do not edit by hand.\n"
        f"export const WORKSPACE_V2_PUBLIC_METHOD_SCOPES = {scopes} as const;\n\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        actual_web = (
            WEB_PUBLIC_METHOD_SCOPES.read_text(encoding="utf-8")
            if WEB_PUBLIC_METHOD_SCOPES.exists()
            else ""
        )
        if actual_web != encode_renderer_public_method_scopes():
            raise SystemExit(
                "desktop/web-grid/src/contracts/generated/workspaceV2RpcCapabilities.ts is stale"
            )
        return 0
    OUTPUT.write_text(expected, encoding="utf-8", newline="\n")
    WEB_PUBLIC_METHOD_SCOPES.write_text(
        encode_renderer_public_method_scopes(),
        encoding="utf-8",
        newline="\n",
    )
    return 0
