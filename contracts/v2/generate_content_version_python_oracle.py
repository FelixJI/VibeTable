"""Replay the fixed Python Content Version producer and compare every frozen sample."""
...
ORACLE = ROOT / "contracts/v2/content-version-python-oracle.json"
CAPTURE = ROOT / "contracts/v2/capture_content_version_python_oracle.py"
BUILD_ROOT = ROOT / "build/contract-oracles/content-version-python"
...
def replay_producer() -> dict[str, object]:
    ...
    if not BUILD_ROOT.resolve().is_relative_to((ROOT / "build").resolve()) or not (
        ...
        raise ValueError("Content Version oracle build directory escapes the worktree")
...
def verify_capture(captured: dict[str, object], oracle: Path = ORACLE) -> None:
    ...
    if json.dumps(current, sort_keys=True, ensure_ascii=False) != json.dumps(
        ...
        raise ValueError("Content Version oracle differs from the fixed Python producer replay")
    if current.get("producer") != PRODUCER or not current.get("cases"):
        raise ValueError("The fixed Python Content Version corpus is incomplete")
...
def main() -> None:
    ...
    print("Validated Python Content Version samples by exact historical producer replay.")
