"""Replay the remaining Python page contract against the immutable query window oracle."""
...
PYTHON_REPLAY_METHODS = ("query.page",)
...
def main() -> int:
    ...
    if render(frozen) != render(asyncio.run(capture())):
        parser.error(
            "Python page contract differs from the frozen producer; inspect the change, do not regenerate"
        )
    return 0
