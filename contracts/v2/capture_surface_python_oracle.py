...
def check_producer(root: Path):
    for symbol, expected in [
        (SurfaceService, SOURCES[1]),
        (_register_surface_methods, SOURCES[0]),
        (RpcDispatcher, SOURCES[5]),
    ]:
        if Path(inspect.getfile(symbol)).resolve() != root / expected:
            raise RuntimeError("Surface capture imported a different Python producer")


async def capture(root: Path):
    check_producer(root)
    samples = cases()
