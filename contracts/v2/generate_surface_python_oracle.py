"""Replay the fixed Python Interface producer and compare every frozen sample."""
...
ORACLE = ROOT / "contracts/v2/surface-python-oracle.json"
CAPTURE = ROOT / "contracts/v2/capture_surface_python_oracle.py"
BUILD_ROOT = ROOT / "build/contract-oracles/surface-python"
...
def replay_producer() -> dict[str, object]:
    ...
    if not BUILD_ROOT.resolve().is_relative_to((ROOT / "build").resolve()) or not (
        ...
        raise ValueError("Surface oracle build directory escapes the worktree")
...
def verify_capture(captured: dict[str, object], oracle: Path = ORACLE) -> None:
    ...
    if json.dumps(current, sort_keys=True, ensure_ascii=False) != json.dumps(
        ...
        raise ValueError("Surface oracle differs from the fixed Python producer replay")
    if current.get("producer") != PRODUCER or len(current.get("cases", [])) != 78:
        raise ValueError("The fixed Python Interface corpus is incomplete")
...
def main() -> None:
    ...
    print("Validated all 78 Python Interface samples by exact historical producer replay.")
