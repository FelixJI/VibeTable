...
<p>Comma-separated list of packages whose install-time lifecycle scripts
(<code>preinstall</code>, <code>install</code>, <code>postinstall</code>, and <code>prepare</code> for non-registry
dependencies) are allowed to run.</p>
<p>This setting is intended for one-off and global contexts: <code>npm exec</code>, <code>npx</code>,
and <code>npm install -g</code>, where no project <code>package.json</code> is involved. For
team-wide policy in a project, use the <code>allowScripts</code> field in
<code>package.json</code> (which also supports explicit denials), or configure it in
<code>.npmrc</code>. Passing <code>--allow-scripts</code> on the command line during a
project-scoped <code>npm install</code>, <code>ci</code>, <code>update</code>, or <code>rebuild</code> is an error.</p>
...
setting.</p>
<h4 id="dangerously-allow-all-scripts"><code>dangerously-allow-all-scripts</code></h4>
<ul>
<li>Default: false</li>
<li>Type: Boolean</li>
</ul>
<p>If <code>true</code>, bypass the <code>allowScripts</code> policy entirely and run every
...
<code>--ignore-scripts</code> still takes precedence over this setting.</p>
