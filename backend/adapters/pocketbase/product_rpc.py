...
ProductRpcHandler = Callable[[ProductParams], Awaitable[dict[str, Any]]]
    ...
    def __init__(
        ...
        *,
        ...
        self._handlers: dict[str, ProductRpcHandler] = {
            "field.settings.describe": self.describe_field_settings,
            "field.change.plan": self.plan_field_change,
            "field.change.apply": self.apply_field_change,
            "field.change.status": self.field_change_status,
            "field.change.cancel": self.cancel_field_change,
            "field.recycleBin.list": self.list_recycled_fields,
            "schema.validate": self.validate_schema,
            "schema.apply": self.apply_schema,
            "schema.delete": self.delete_schema,
            "schema.list": self.list_tables,
            "schema.getTable": self.get_table_schema,
            "schema.describe": self.describe_schema,
            "query.page": self.query_page,
            "query.view": self.query_view,
            "query.readRows": self.read_rows,
            "query.validateSnapshot": self.validate_snapshot,
            "mutation.preview": self.preview_mutation,
            "mutation.apply": self.apply_mutation,
            "formula.validate": self.validate_formula,
            "formula.draft.validate": self.validate_formula_draft,
            "formula.preview": self.preview_formula,
            "file.list": self.list_attachment_refs,
            "file.token": self.create_file_token,
            "file.applyHostChange": self.apply_host_attachment_change,
            "file.saveHostFile": self.save_attachment_to_host,
            "history.read": self.read_history,
            "history.previewRestore": self.preview_history_restore,
            "history.applyRestore": self.apply_history_restore,
            "events.reconcile": self.reconcile,
            "relation.searchTargets": self.search_relation_targets,
            "relation.createTarget": self.create_relation_target,
            "relation.updateSingle": self.update_single_relation,
            "relation.previewDelta": self.preview_relation_delta,
            "relation.applyDelta": self.apply_relation_delta,
            "lookup.list": self.list_lookups,
            "lookup.validate": self.validate_lookup,
            "lookup.preview": self.preview_lookup,
            "lookup.query": self.query_lookups,
            "lookup.valuePage": self.lookup_value_page,
        }
        if self._handlers.keys() != PRODUCT_RPC_REGISTRY.keys():
