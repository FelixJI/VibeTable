...
func queryPageHTTPMux(t *testing.T, pb *pocketbase.PocketBase, registration productrpc.Registration) http.Handler {
	...
	dispatcher, err := productrpc.New(productrpc.Identity{
		...
		registration, queryViewRegistration(unrelatedViewMustNotRun{t: t}), relationPreviewDeltaRegistration(unrelatedRelationPreviewMustNotRun{t: t}), schemaDescribeRegistration(pb, relation.New(pb, nil, nil)), schemaGetTableRegistration(pb),
