...
func queryPageHTTPMux(t *testing.T, pb *pocketbase.PocketBase, registration productrpc.Registration) http.Handler {
	...
	dispatcher, err := productrpc.New(productrpc.Identity{
		...
		registration, querySelectionOpenRegistration(unrelatedSelectionMustNotRun{t: t}), schemaDescribeRegistration(pb, relation.New(pb, nil, nil)), schemaGetTableRegistration(pb),
