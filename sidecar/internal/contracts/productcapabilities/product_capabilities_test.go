...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 15 ||
		...
		got[8].Method != "query.selectionOpen" || got[9].Method != "query.view" || got[10].Method != "relation.previewDelta" || got[11] != (RPCDescriptor{Method: "relation.searchTargets", Scope: WorkspaceScope, Audience: RendererPublic, CapabilityID: "relation.lookup", Owner: GoSidecar, Effect: ReadEffect}) || got[12].Method != "schema.describe" ||
		got[13].Method != "schema.getTable" || got[14].Method != "schema.list" {
