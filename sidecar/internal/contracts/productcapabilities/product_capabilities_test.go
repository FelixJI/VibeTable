...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 13 ||
		...
		got[8].Method != "query.selectionOpen" || got[9].Method != "query.view" || got[10].Method != "schema.describe" ||
		got[11].Method != "schema.getTable" || got[12].Method != "schema.list" {
