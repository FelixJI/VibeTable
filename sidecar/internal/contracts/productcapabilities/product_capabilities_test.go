...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 21 ||
		...
		got[13].Method != "query.selectionOpen" || got[14] != (RPCDescriptor{Method: "query.validateSnapshot", Scope: WorkspaceScope, Audience: RendererPublic, CapabilityID: "schema.query", Owner: GoSidecar, Effect: ReadEffect}) || got[15].Method != "query.view" || got[16].Method != "relation.previewDelta" || got[17] != (RPCDescriptor{Method: "relation.searchTargets", Scope: WorkspaceScope, Audience: RendererPublic, CapabilityID: "relation.lookup", Owner: GoSidecar, Effect: ReadEffect}) || got[18].Method != "schema.describe" ||
		got[19].Method != "schema.getTable" || got[20].Method != "schema.list" {
