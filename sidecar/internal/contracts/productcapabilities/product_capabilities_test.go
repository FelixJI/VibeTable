...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 10 ||
		...
		}) || got[3].Method != "lookup.list" || got[4].Method != "query.page" ||
		got[5].Method != "query.view" ||
		got[6] != (RPCDescriptor{Method: "relation.searchTargets", Scope: WorkspaceScope, Audience: RendererPublic,
			CapabilityID: "relation.lookup", Owner: GoSidecar, Effect: ReadEffect}) ||
		got[7].Method != "schema.describe" || got[8].Method != "schema.getTable" || got[9].Method != "schema.list" {
