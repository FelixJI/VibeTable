...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 12 ||
		...
		got[8].Method != "query.selectionOpen" || got[9].Method != "schema.describe" ||
		got[10].Method != "schema.getTable" || got[11].Method != "schema.list" {
