...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 7 ||
		got[0].Method != "events.reconcile" || got[1].Method != "file.list" ||
		got[2].Method != "lookup.list" || got[3].Method != "query.readRows" || got[4].Method != "schema.describe" ||
		got[5].Method != "schema.getTable" || got[6].Method != "schema.list" {
