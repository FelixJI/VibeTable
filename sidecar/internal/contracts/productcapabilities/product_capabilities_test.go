...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 16 ||
		...
		}) || got[3].Method != "lookup.list" || got[4] != (RPCDescriptor{Method: "lookup.query", Scope: WorkspaceScope, Audience: RendererPublic, CapabilityID: "relation.lookup", Owner: GoSidecar, Effect: ReadEffect}) || got[5].Method != "query.cursorFetch" || got[6].Method != "query.cursorOpen" || got[7].Method != "query.page" || got[8].Method != "query.readRows" ||
