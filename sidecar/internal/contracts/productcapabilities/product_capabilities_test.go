...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 9 ||
		...
		}) || got[3].Method != "lookup.list" || got[4].Method != "query.cursorFetch" ||
		got[5].Method != "query.cursorOpen" || got[6].Method != "schema.describe" ||
		got[7].Method != "schema.getTable" || got[8].Method != "schema.list" {
