...
func TestGeneratedRPCDescriptorsKeepCanonicalPolicyAndReturnCopies(t *testing.T) {
	...
	if got := CurrentOwnerRPCDescriptors(GoSidecar); len(got) != 18 ||
		...
		got[11].Method != "query.selectionOpen" || got[12].Method != "query.view" || got[13].Method != "relation.previewDelta" || got[14] != (RPCDescriptor{Method: "relation.searchTargets", Scope: WorkspaceScope, Audience: RendererPublic, CapabilityID: "relation.lookup", Owner: GoSidecar, Effect: ReadEffect}) || got[15].Method != "schema.describe" ||
