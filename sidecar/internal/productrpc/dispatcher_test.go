...
func TestNewRequiresRegistrationsToExactlyMatchGeneratedGoSidecarPolicy(t *testing.T) {
	...
	if methods := dispatcher.Methods(); len(methods) != 13 ||
		...
		methods[8].Method != "query.selectionOpen" || methods[9].Method != "query.view" || methods[10].Method != "schema.describe" ||
		methods[11].Method != "schema.getTable" || methods[12].Method != "schema.list" {
