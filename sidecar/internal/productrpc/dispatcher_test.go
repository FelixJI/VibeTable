...
func TestNewRequiresRegistrationsToExactlyMatchGeneratedGoSidecarPolicy(t *testing.T) {
	...
	if methods := dispatcher.Methods(); len(methods) != 21 ||
		...
		methods[13].Method != "query.selectionOpen" || methods[14] != (Method{Method: "query.validateSnapshot", Scope: productcapabilities.WorkspaceScope}) || methods[15].Method != "query.view" || methods[16].Method != "relation.previewDelta" || methods[17] != (Method{Method: "relation.searchTargets", Scope: productcapabilities.WorkspaceScope}) || methods[18].Method != "schema.describe" ||
		methods[19].Method != "schema.getTable" || methods[20].Method != "schema.list" {
