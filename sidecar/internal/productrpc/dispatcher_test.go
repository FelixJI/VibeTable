...
func TestNewRequiresRegistrationsToExactlyMatchGeneratedGoSidecarPolicy(t *testing.T) {
	...
	if methods := dispatcher.Methods(); len(methods) != 9 ||
		...
		methods[3].Method != "lookup.list" || methods[4].Method != "query.cursorFetch" ||
		methods[5].Method != "query.cursorOpen" || methods[6].Method != "schema.describe" ||
		methods[7].Method != "schema.getTable" || methods[8].Method != "schema.list" {
