...
// VerifyProtectionSnapshot proves that a caller-supplied protection snapshot
// is intact and bound to the current writable authority and mutation revision.
func (runtime *Runtime) VerifyProtectionSnapshot(
	ctx context.Context,
	snapshotID string,
) error {
	if runtime == nil || !validUUID(snapshotID) {
		return errors.New("snapshot.protection_invalid")
	}
	record, err := runtime.snapshotRecord(ctx, snapshotID)
	if err != nil {
		return err
	}
	...
	if !valid ||
		record.WorkspaceID != token.WorkspaceID ||
		record.FenceEpoch != token.FenceEpoch ||
		record.ClaimID != token.ClaimID ||
		record.MutationRevision != counters.MutationRevision {
		return errors.New("snapshot.protection_stale")
	}
	return nil
}
