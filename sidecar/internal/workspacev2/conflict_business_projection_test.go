...
func TestConflictBusinessAndSharedSettingsPublicApplyPreservesRecovery(t *testing.T) {
	...
	runtime, err := Open(ctx, Options{App: app, DataDir: dataDir, WorkspaceID: testWorkspaceID, SessionEpoch: 7, FenceEpoch: 3, ClaimID: testClaimID, Ledger: ledger, RequestShutdown: func() {}, DeferBackgroundWorkers: true})
	requireSnapshotRestore(t, err)
	...
	capture := func(marker string) (snapshot.Record, conflictresolution.Candidate) {
		...
		record, captured, err := runtime.snapshots.Capture(ctx, snapshot.CaptureRequest{WorkspaceID: testWorkspaceID, Authority: token.Authority(), Trigger: snapshot.TriggerManual, Pinned: true})
		requireSnapshotRestore(t, err)
		if !captured {
	...
	expectedRecovery := []string{localSnapshot.SnapshotID, replicaSnapshot.SnapshotID}
