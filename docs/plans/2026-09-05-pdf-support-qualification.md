...
`noTextLayer`；这已满足“进入决策”的条件，但尚无合格引擎或采纳结论。当前基础测试验证了普通文本、Flate
与含 UTF-16 BOM 的中文字符串解码；即使该样本声明 Type0/ToUnicode，现有解码器也没有读取字体映射，不能据此
证明一般 Type0/ToUnicode 中文支持。后续资格样本必须覆盖需要实际字体映射的非恒等字码；候选探针的 Type0
乱码仍是该候选不合格的功能观察，不是产品能力结论。
