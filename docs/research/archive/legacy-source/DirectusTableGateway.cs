...
public sealed class DirectusTableGateway : ITableRpcGateway, IDisposable
{
    ...
    {
        // A newly started backend only knows the static capability manifest.
        // Runtime-created collections are restored from Directus by identifier
        // reconciliation. Do that before the first list call so database.opened
        // never publishes a stale table list while the mapping registry already
        // shows the persisted table and fields.
        await _directus.ReconcileIdentifierMappingsAsync(token);
