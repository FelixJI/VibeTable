...
{
    [TestMethod]
    public async Task OpenDatabase_ReconcilesMappingsBeforeListingCollections()
    {
        using var directus = new FakeDirectusGateway();
        var gateway = new DirectusTableGateway(directus, new FakeLocalStateGateway());

        await gateway.OpenDatabaseAsync("directus://configured", CancellationToken.None);

        CollectionAssert.AreEqual(
            new[] { "reconcile", "list" },
            directus.CollectionDiscoveryCalls.ToArray());
    }
    ...
    private sealed class FakeDirectusGateway : IDirectusRpcGateway
    {
        ...
        public List<string> CollectionDiscoveryCalls { get; } = [];
        ...
        {
            CollectionDiscoveryCalls.Add("list");
            return Task.FromResult(new DirectusCollectionList(
                new[] { "vibetable_demo" },
        ...
        {
            CollectionDiscoveryCalls.Add("reconcile");
            return Task.FromResult(new IdentifierMappingsResult(Array.Empty<IdentifierMappingEntry>()));
        }
