...
现行 Product owner 以[生成能力清单](../../contracts/v2/product-rpc-capability-manifest.json)和[ownership inventory](../../contracts/v2/product-runtime-ownership-inventory.json)为准。`query.readRows`、`query.cursorOpen` 与 `query.cursorFetch` 按该 policy 直达 Go，Python 不再注册这些方法。仍属 Python 的 `selectionOpen` 产生的 cursor 可由 Go 续读。
...
生成 policy 按方法选择 Go 或 Python，并验证 `query.readRows` 通过 Product HTTP gateway 保持返回值。游标组合测试另验证 Python selection 与 Go fetch 的 cursor 传递、代际租约及零 fallback。
