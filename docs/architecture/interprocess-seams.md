...
现行 Product 方法的 owner 以 [生成能力清单](../../contracts/v2/product-rpc-capability-manifest.json) 为准，迁移边界与剩余切片见 [ownership inventory](../../contracts/v2/product-runtime-ownership-inventory.json)。`query.cursorOpen` 与 `query.cursorFetch` 按该 policy 直达 Go；仍属 Python 的 `selectionOpen` 产生的 cursor 可由 Go 续读。
...
生成 policy 按方法选择 Go 或 Python。游标组合测试另验证 Python selection 与 Go fetch 的 cursor 传递、代际租约及零 fallback。
