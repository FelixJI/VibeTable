...
- 纠正后，`uv run --frozen --no-sync python -m pytest tests/e2e/test_directory_replica_payloads.py tests/e2e/test_packaged_replica_hosts.py tests/e2e/test_product_e2e_runner.py tests/contract/test_product_e2e_capability_index.py --no-cov -q`：218 PASS，9.50s，`fixtures-corrected.log`。后续追加解决/重启失败回归及主干同步的最终结果另记。
