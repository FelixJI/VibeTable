...
| 目录镜像 Workspace/Replica 恢复 | `workspacev2`、directory replica provider | `ProductionWorkspaceRuntime`、Workspace V2 dispatcher | Workspace Center、Tables、Storage UI | 目录镜像拓扑及公开 workspace/storage 方法；不广告 exclusive-writer | `23-directory-replica-recovery`：创建、写入、释放、同 UUID 重开、sidecar kill 后记录/revision/replica 状态保持 | **已有通过样本，稳定性待验收**；后续 CI 场景 23 失败尚未关闭，见[证据与限制](../e2e-performance.md#当前产品-e2e-证据)。手动同步继续 Internal，跨设备 reconnect/冲突资格不在本结论内。 |
