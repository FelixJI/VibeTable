...
| 目录镜像 Workspace/Replica 恢复 | `workspacev2`、directory replica provider | `ProductionWorkspaceRuntime`、Workspace V2 dispatcher | Workspace Center、Tables、Storage UI | 目录镜像拓扑及公开 workspace/storage 方法；不广告 exclusive-writer | `23-directory-replica-recovery`：创建、写入、释放、同 UUID 重开、sidecar kill 后记录/revision/replica 状态保持 | **Closed（场景声明范围）**；手动同步继续 Internal，跨设备 reconnect/冲突资格不在本结论内。 |
