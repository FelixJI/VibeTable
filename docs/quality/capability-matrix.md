...
| Retention / Repository | `workspacev2/retention_*`、`repository_*` | `MainWindow.Product.cs` capability 合成/dispatcher | `WorkspaceProtectionSettings.vue` | `retention.policy.v2`、`repository.verify` | `13-protection-policy`：策略更新/旧 revision 拒绝、verify、零删除 plan/apply；`23-retention-nonzero-apply`：空 catalog 中两个未固定快照的非零逻辑 tombstone、最新仍可达且至少一个较旧快照不可达、3 个月 grace 内 `reclaimedBytes=0`、二次 plan/apply 删除 0 | **Pending main E2E evidence**；23 仅在 PR 场景定义中验证隔离候选与逻辑删除，未覆盖物理 Sweep，必须由合并后的可信 main 打包运行关闭。 |
