...
| 目录镜像 Workspace/Replica 恢复 | `workspacev2`、directory replica provider | `ProductionWorkspaceRuntime`、Workspace V2 dispatcher | Workspace Center、Tables、Storage UI | 目录镜像拓扑及公开 workspace/storage 方法；不广告 exclusive-writer | `23-directory-replica-recovery`：创建、写入、释放、同 UUID 重开、sidecar kill 后记录/revision/replica 状态保持 | **已有通过样本，稳定性待验收**；后续 CI 场景 23 失败尚未关闭，见[证据与限制](../e2e-performance.md#已通过的产品-e2e-样本)。手动同步继续 Internal，跨设备 reconnect/冲突资格不在本结论内。 |
| Replica synchronize | `workspacev2/replica_*` | `ProductionWorkspaceHooks` 是保留 consumer；Host raw route 拒绝并返回 `CAPABILITY_NOT_PUBLIC` | 生成的 public method map 令 Web preflight 返回 `workspace.method_not_public`；`WorkspaceProtectionSettings.vue` 不挂载手动同步按钮 | manifest 标记 `rendererInternal`，不进入生成的 renderer-public map | raw route 拒绝测试；`23-directory-replica-recovery` 仅证明目录副本恢复，不包含手动同步 | **Internal only**；不把内部协调钩子广告成用户能力。 |
...
| Formula / Relation / Lookup | Go formula、relation、lookup、Mutation Kernel 与 recalculation jobs | 现有 Product RPC bridge；部分方法仍经 Python 注册/适配 | Formula editor、Relation/Lookup editor、grid renderer | Product RPC policy 的 `schema.formula`、`relation.lookup`；公开方法闭集见[资格规范](formula-relation-qualification.md) | `05-formula-lifecycle`（`formula.recalculation`）只覆盖字段迁移；`06-relation-fanout`（`relation.fanout`）只覆盖 cascade 影响预览 | **Partial / Open**；内核基础存在，但作者文档、单一 Go owner、统一依赖计划、完整产品链、N+1/规模与当前格式恢复资格未闭合，不得标记 Closed。 |
