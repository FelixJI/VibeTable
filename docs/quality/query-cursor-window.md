...
本片将“打开并续读一个游标窗口”作为完整意图，只切换 query.cursorOpen、query.cursorFetch。WPF 读取生成 policy，Go Product adapter 调用既有 Query Port；对应 Python handler/注册同时删除，没有生产 fallback。selectionOpen 仍由 Python adapter 处理。游标是既有领域签名及修订绑定令牌，本片不新增内存游标注册表、过期策略或并发状态机。
...
当前本地切片仍依赖PR275冻结语料。前置正式合入后必须按最新main串行整合，保留readRows/page/history的已完成owner、重生成共享能力清单，完成最终双轴审查、fresh CI、squash及post-main CI/CD。当前不发布新的待合并PR，不将未完成门禁标绿。
