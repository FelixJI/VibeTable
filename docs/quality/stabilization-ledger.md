...
- 旧内部候选：`ff95d26d827939335e4c903c889f00fc0b97abf1`（`feat: complete workspace snapshots and file history`）。
- application version：`0.1.0`；sidecar migration manifest `schemaVersion=5`。
- 当前基线：application version `0.5.0`；sidecar migration manifest `schemaVersion=7`。
- 选择理由：该 commit 是当前 main 的祖先，已具备真实 workspace、Snapshot 与 FileHistory，且尚不含
