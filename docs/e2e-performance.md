...
- 当前 manifest gap：3（`26-lookup-definition-read`、`28-relation-delta-preview`、`29-lookup-source-pagination`）。
- 当前 manifest surplus：无。

场景 "26-lookup-definition-read"、"28-relation-delta-preview" 与 "29-lookup-source-pagination" 已进入 manifest，尚无覆盖它们的正式 main 打包报告；上述 23/23 历史样本不包含这些新增场景。
