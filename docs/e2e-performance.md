...
- 当前 manifest gap：3（`26-lookup-definition-read`、`27-relation-target-search`、`28-relation-delta-preview`）。
- 当前 manifest surplus：无。

场景 "26-lookup-definition-read"、"27-relation-target-search" 和 "28-relation-delta-preview" 已进入 manifest，尚无覆盖它们的正式 main 打包报告；上述 23/23 历史样本不包含这三项新增场景，也不证明关系目标搜索资格。
