...
- 当前 manifest gap：2（`23-directory-replica-recovery`、`26-lookup-definition-read`）。
...
这些新增场景已进入 manifest，但尚无覆盖它们的同一正式 main `required` 打包报告；
当前权威基线仍是上述 22/22，不得提前宣称当前 manifest 已闭环。
