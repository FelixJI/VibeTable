...
describe("ImportRelationMapping", () => {
  ...
  it("marks duplicate and empty source headers as unselectable", () => {
    const wrapper = mount(ImportRelationMapping, {
      props: baseProps({
        modelValue: [{ sourceColumn: "number", targetField: "partner", matchField: "fld_code" }],
      }),
    });
  ...
  it("adds a complete mapping row with stable identities and defaults", async () => {
    ...
    expect(emitted![0]).toEqual([[
      { sourceColumn: "number", targetField: "partner", matchField: "fld_code" },
    ]]);
  });
  ...
  it("exposes only unique-enabled match fields of the selected target", () => {
    const wrapper = mount(ImportRelationMapping, {
      props: baseProps({
        ...
        modelValue: [{ sourceColumn: "Partner Code", targetField: "partner", matchField: "fld_code" }],
      }),
    });
    ...
    expect(targetOptions.find((item) => item.value === "owner")?.disabled).toBe(true);
    expect(targetOptions.find((item) => item.value === "partner")?.disabled).toBe(true);
  });
