...
public sealed class WebMessageRouterTests
{
    ...
    public void WhitelistsAcceptClosedProductDataUseCasesAndRejectProviderMethods()
    {
        ...
        foreach (string type in ProductDataRpcRegistry.RequestTypes)
        {
            ...
            if (type is "events.reconcile" or "file.list" or "lookup.list" or "query.page" or "query.view" or "schema.describe" or "schema.getTable")
            {
                request["scope"] = new
                {
                    sequence = 0,
    ...
    public void RelationGoRouteRequiresScopeAndPreservesItsWire()
    {
        ...
        HostReplyMessage? reply = router.Route(
            """{"type":"relation.previewDelta","requestId":"relation-go","payload":{"relationId":"customer"}}""");

        Assert.AreEqual("BAD_WORKSPACE_SCOPE", reply?.Payload?.Code);
        Assert.HasCount(0, dispatched);
        const string scoped = """
            {"type":"relation.previewDelta","requestId":"relation-go","payload":{"relationId":"customer"},"scope":{"scope":"workspace","workspaceId":"11111111-1111-4111-8111-111111111111","sessionEpoch":7,"operationId":"22222222-2222-4222-8222-222222222222","sequence":1}}
            """;
    ...
    public void GeneratedProductPolicyDoesNotHideExistingPublicTypedRoutes()
    {
        ...
        foreach (string route in ProductDataRpcRegistry.RequestTypes)
        {
            ...
            Assert.AreEqual(
                route is "events.reconcile" or "file.list" or "lookup.list" or "query.page" or "query.view" or "schema.describe" or "schema.getTable"
                    ? "goSidecar"
                    : "pythonBff",
                capability.Owner,
                route);
        ...
        foreach (string route in RelationLookupRpcRegistry.RequestTypes)
        {
            Assert.IsTrue(policy.TryGet(route, out ProductRpcCapability capability), route);
            Assert.AreEqual("rendererPublic", capability.Audience, route);
            Assert.AreEqual(route == "relation.previewDelta" ? "goSidecar" : "pythonBff",
