...
public sealed class WebMessageRouterTests
{
    ...
    public void WhitelistsAcceptClosedProductDataUseCasesAndRejectProviderMethods()
    {
        ...
        foreach (string type in ProductDataRpcRegistry.RequestTypes)
        {
            ...
            if (type is "events.reconcile" or "file.list" or "lookup.list" or "query.readRows" or "schema.describe" or "schema.getTable")
            {
                request["scope"] = new
                {
                    sequence = 0,
    ...
    public void GeneratedProductPolicyDoesNotHideExistingPublicTypedRoutes()
    {
        ...
        foreach (string route in ProductDataRpcRegistry.RequestTypes)
        {
            ...
            Assert.AreEqual(
                route is "events.reconcile" or "file.list" or "lookup.list" or "query.readRows" or "schema.describe" or "schema.getTable"
                    ? "goSidecar"
                    : "pythonBff",
                capability.Owner,
                route);
